<template>
    ...
    <div class="content">
      ...
      <div class="input">
        ...
        <div class="regions animated" v-show="choseRegion" transition="flipInX">
          <div class="region" v-on:click.stop="choseRegion=false">中国大陆</div>
          <div class="region" v-on:click.stop="choseRegion=false">香港特区</div>
          <div class="region" v-on:click.stop="choseRegion=false">澳门特区</div>
          <div class="region" v-on:click.stop="choseRegion=false">台湾地区</div>
          <div class="region" v-on:click.stop="choseRegion=false">美国</div>
          <div class="region" v-on:click.stop="choseRegion=false">日本</div>
          <div class="region" v-on:click.stop="choseRegion=false">韩国</div>
          <div class="region" v-on:click.stop="choseRegion=false">新加坡</div>
          <div class="region" v-on:click.stop="choseRegion=false">德国</div>
          <div class="region" v-on:click.stop="choseRegion=false">英国</div>
          <div class="region" v-on:click.stop="choseRegion=false">法国</div>
          <div class="region" v-on:click.stop="choseRegion=false">加拿大</div>
      ...
      <div class="btn btn-red" v-on:click.stop="go({from:'isReg',to:'showSmp'})" ng-click="showSmp=true;closeReg=true">注册</div>
      <p class="oauth_p">第三方账号登陆</p>

      <div class="oauth_panel">
        <i class="inline-block sp i156" v-on:click.stop="" ng-click="closeReg=true;isOauth1=true;"></i>
  ...
  <div class="modal statement-panel animated" v-show="showSmp" transition="fade">
    ...
    <div class="sel-profession">
      <div class="sel-box" v-on:click.stop="showPro=true">请选择您的职业</div>
      <i class="sp i3 block"></i>
      <i class="sp i2 block"></i>

      <div class="profession-panel animated"  v-show="showPro" transition="flipInX">
        <div class="profession" v-on:click.stop="showPro=false">导演</div>
        <div class="profession" v-on:click.stop="showPro=false">编剧</div>
        <div class="profession" v-on:click.stop="showPro=false">摄影师</div>
        <div class="profession" v-on:click.stop="showPro=false">剪辑师</div>
        <div class="profession" v-on:click.stop="showPro=false">调色师</div>
        <div class="profession" v-on:click.stop="showPro=false">灯光师</div>
        <div class="profession" v-on:click.stop="showPro=false">发烧友</div>
        <div class="profession" v-on:click.stop="showPro=false">自定义</div>
      </div>
    </div>
    <div class="btn btn-red btn-red2" v-on:click.stop="closeModal()">同意并注册</div>
  </div>
  <div class="modal reg-panel login reg-panel-2 animated" v-show="isLog" transition="fade">
    <div class="head">
      <i class="block sp i183 r-logo"></i>
      <h3>欢迎回来</h3>
      <i class="block close sp i103" v-on:click.stop.stop="closeModal()"></i>
    </div>
    <div class="content">
      <p class="hello">一日不急 如三秋兮</p>

      <div class="input">
        <span class="i-head switch-type" v-on:click.stop.stop="showSwitchs=true">昵称</span>
        <input type="text" placeholder="">
        <span class="status error"></span>
        <i class="sp i3 block"></i>
        <i class="sp i2 block"></i>
        ...
        <div class="switch-items animated" v-show="showSwitchs" transition="flipInX">
          <div class="switch-item" v-on:click.stop.stop="showSwitchs=false">昵称</div>
          <div class="switch-item" v-on:click.stop.stop="showSwitchs=false">手机</div>
          <div class="switch-item" v-on:click.stop.stop="showSwitchs=false">邮箱</div>
        </div>
      </div>
      <div class="input">
        <span class="i-head">密码</span>
        <input type="{{showPass}}"  placeholder="" ng-model="password"
               v-on:focus="showComplexity=true" v-on:blur="showComplexity=false">
        <i class="block sp i32 eye" v-bind:class="{'i33':showPass==='text'}" v-on:click.stop.stop="showPassToggle()"></i>
        <span class="status sp i75 suc"></span>
      </div>
      <div class="input min">
        <p class="forget-pass" v-on:click.stop.stop="go({from:'isLog',to:'isReset1'})">忘记密码?</p>
      </div>
      <div class="btn btn-red" v-on:click.stop.stop="closeModal()">登陆</div>
      <p class="go-reg">没有账号?去<span v-on:click.stop.stop="go({from:'isLog',to:'isReg'})">注册</span></p>
      <p class="oauth_p">第三方账号登陆</p>
      <div class="oauth_panel">
  ...
  <div class="modal reg-panel reset-pass reg-panel-2 animated"
    ...
    <div class="content">
      ...
      <div class="btn-wrap">
        <div class="btn btn-blue inline-block" v-on:click.stop="go({from:'isReset1',to:'isReset2'})">手机验证</div>
        <div class="btn btn-blue inline-block" v-on:click.stop="go({from:'isReset1',to:'isReset3'})">邮箱验证</div>
      </div>
  ...
  <div class="modal reg-panel reset-pass login reg-panel-2 animated"
    ...
    <div class="content">
      ...
      <div class="btn btn-red" v-on:click.stop="go({from:'isReset2',to:'isReset5'})">下一步</div>
    </div>
  </div>
  <div class="modal reg-panel reset-pass login reg-panel-2 animated"
    ...
    <div class="content">
      ...
      <div class="btn btn-red mt20" v-on:click.stop="go({from:'isReset3',to:'isReset4'})">下一步</div>
    </div>
  </div>
  <div class="modal reg-panel reset-pass login reg-panel-2 animated"
    ...
    <div class="content">
      <div class="des-info">
        ...
        <p class="link" v-on:click.stop="closeModal()">去邮箱查看></p>
      </div>
  ...
  <div class="modal reg-panel reset-pass login reg-panel-2 animated"
    ...
    <div class="content">
      ...
      <div class="btn btn-red" v-on:click.stop="go({from:'isReset5',to:'isReset6'})">确定</div>
    </div>
  </div>
  <div class="modal reg-panel reset-pass login reg-panel-2 animated"
    ...
    <div class="content">
      ...
      <div class="btn btn-red" v-on:click.stop="closeModal()">完成</div>
    </div>
  </div>
